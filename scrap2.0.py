import requests
import json
import os
from bs4 import BeautifulSoup


BASE = "Scrap/"
COMPUTER_DEPARTMENT_ID = "2903"

if not os.path.isdir(BASE):
    os.mkdir(BASE)


cookies = {
    'ASP.NET_SessionId': 'zvcvom4rqmcar4qz5d1akbo3',
    'Sess_ID_Sess': 'njh1ldgxmu2fr3tvwk3kakzn',
    'crisp-client^%^2Fsession^%^2Faafc7d8c-2085-4323-b1b1-3e4ff9e56af1': 'session_4b1e82d1-d0cd-4274-8efd-6b44194a9af6',
    'crisp-client^%^2Fsession^%^2Fc4aa641f-a768-4098-980d-ec75542d273d': 'session_cb67904e-20a9-4de0-a1d4-ba56c28186c9',
}


headers = {
    'authority': 'sess.shirazu.ac.ir',
    'cache-control': 'max-age=0',
    'sec-ch-ua': '^\^',
    'sec-ch-ua-mobile': '?0',
    'sec-ch-ua-platform': '^\^Windows^^',
    'upgrade-insecure-requests': '1',
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.51 Safari/537.36',
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
    'sec-fetch-site': 'same-origin',
    'sec-fetch-mode': 'navigate',
    'sec-fetch-user': '?1',
    'sec-fetch-dest': 'document',
    'referer': 'https://sess.shirazu.ac.ir/Sess/277111379502',
    'accept-language': 'en-US,en;q=0.9',
}


def scrap_departments():
    response = requests.get('https://sess.shirazu.ac.ir/Sess/3339122734187', headers=headers, cookies=cookies)
    soup = BeautifulSoup(response.text, "html.parser")

    departments = []
    deps = soup.find(id="edDepartment")
    for dep in deps.contents:
        if dep == "\n":
            continue
        departments.append({
            "value": dep['value'],
            "title": dep.string,
            })
    f = open(BASE + 'departments.txt', 'w')
    f.write(json.dumps(departments))
    f.close()


def load_departments():
    try:
        f = open(BASE + 'departments.txt', 'r')
    except:
        scrap_departments()

    f = open(BASE + 'departments.txt', 'r')
    departments = json.loads(f.read())
    f.close()

    return departments


def scrap_courses_of_department(semester, department_id=COMPUTER_DEPARTMENT_ID):

    data = {
      '__VIEWSTATE': '',
      '__VIEWSTATEGENERATOR': 'D02604AD',
      '__EVENTVALIDATION': '/wEdAAXOX7L/8YlPu3I5g5xz+NgU6/R1CdSTRgr1D4vCUegGUZFB0O/9E5xLpcfo0i3k0CuMtpATfle4RqQS6XMgMUtAS1f8aOOWPGzSVWaY7KNHgCN5yZ9vZceCZ1RL93GgH9t6iiBS1xNl+oO6GgDwXjGc',
      'Banner1:zzChanel': '',
      'edSemester': semester,
      'edDepartment': department_id,
      'Channel': 'Act=List;'
    }

    response = requests.post('https://sess.shirazu.ac.ir/Sess/3022750784321', headers=headers, data=data, cookies=cookies)
    soup = BeautifulSoup(response.text, 'html.parser')
    courses = []
    courses_data = soup.find(id='edList')
    f = open(BASE + semester[:-1] + "_" + semester[-1] + '.txt', 'w', encoding="utf-8")
    for i in courses_data:
        if i == "\n":
            continue;
        name = i.find_all('td')
        f.write(name[1].string + '\n')


def scrap_department_for_several_terms(department_id=COMPUTER_DEPARTMENT_ID):
    courses = {}
    try:
        start_year = int(input("enter start year as a two digit number:"))
        end_year = int(input("enter end year as a two digit number:"))
    except:
        pirnt("wrong input!")
        scrap_department_for_several_terms(department_id)
    for year in range(start_year, end_year+1):
        print(year)
        year = '13'+str(year)
        for term in ['1','2']:
            scrap_courses_of_department(year+term)
            
    create_report(start_year, end_year)
    

def create_report(start_year, end_year):
    courses = {}

    for year in range(start_year, end_year+1):
        print(year)
        year = '13'+str(year)
        for term in ['1','2']:
            try:
                f = open(BASE + year+"_"+term+'.txt', 'r', encoding="utf-8")
            except:
                scrap_courses_of_department(year+term)
            f = open(BASE + year+"_"+term+'.txt', 'r', encoding="utf-8")

            course_list = f.readlines()
            for course in course_list:
                if course not in courses.keys():
                    courses[course] = set()
                courses[course].add(year+term)

    f = open(f'{BASE}{start_year}_to_{end_year}.txt', 'w', encoding="utf-8")
    for key in courses.keys():
        l = [f'{x[:-1]}_{x[-1]}' for x in list(courses[key])]
        s = ' - '.join(sorted(l, reverse = True))
        s += '\n\n'
        f.write(f'{key} {s}')
    f.close()

